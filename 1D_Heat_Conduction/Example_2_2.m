...
% Parameters 
 ...
 l = 0.02;                                    % Length of a rod(m)   
 dx = 0.004;                                  % Spatial step
 q = 1000*1000;                               % Volumetric heat generation rate(W/m^3)
 TA = 100;                                    % Temperature at A in degree celsius
 TB = 200;                                    % Temperature at B in degree celsius
...
T_exact(n) = TB;
% Exact Solution
% T = [(TB-TA)/L+q/2k(L-x)]x+TA
for i = 2:n-1
    T1 = (TB-TA)/l;
    T2 = (l-x(i))*q/(2*k);
    T_exact(i) = (T1+T2)*x(i)+TA;
end

% Comparison between finite volume solution and the exact solution
for i = 2:n-1
    Error_perc(i) = ((T(i)-T_exact(i))/T_exact(i))*100;
end
 plot(x*100,T,'sb')
 hold on
 plot(x*100,T_exact,'k-','LineWidth',3)
 axis([0 2 50 270])
