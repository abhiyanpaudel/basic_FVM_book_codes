...
for i = 1:length(xcenters)
    if i == 1
        u_dash(i) = (aE(i)*u(i+1))/aP(i);
    elseif i == length(xcenters)
        u_dash(i) = (aW(i)*u(i-1))/aP(i);
    else
        u_dash(i) = (aE(i)*u(i+1)+aW(i)*u(i-1))/aP(i);
    end
end
...
for i = 2:length(xnodes)-1
    ...
    FW(i) = rho*u_dash(i-1)*centerA(i-1);
    FE(i) = rho*u_dash(i)*centerA(i);
    AP(i) = AW(i)+AE(i);
    b(i) = FW(i)-FE(i);
end
...
for i = 1:length(xcenters)
     if i == 1
          uA = u_dash(i)*centerA(i)/nodeA(i);
          Fw = rho*uA*nodeA(i);
          Fe = rho*(u_dash(i)+u_dash(i+1))*0.5*nodeA(i+1);
          aW(i) = 0;
          aE(i) = 0;
          aP(i) = Fe+Fw*0.5*(centerA(i)/nodeA(i))^2;
          Su(i)= (P0-P(i+1))*centerA(i)+Fw*(centerA(i)/nodeA(i))*u(i);
          d(i) = centerA(i)/aP(i);
     elseif i == length(xcenters)
          Fw = rho*(u_dash(i-1)+u_dash(i))*0.5*nodeA(i);
          Fe = rho*u_dash(i)*centerA(i);
          aW(i) = Fw;
          aE(i) = 0;
          aP(i) = aW(i)+aE(i)+(Fe-Fw);
          Su(i)= (P(i)-P(i+1))*centerA(i);
